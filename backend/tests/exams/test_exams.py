...
from tests.dummy_data.exams import make_fake_exam
...
class TestExamsApi:
    ...
    async def test_create_exam_crud_exception(
    ...
    async def test_list_my_exams_not_teacher(
        self, client, override_auth, mock_exams_db
    ):
        override_auth(role="STUDENT")
        mock_exams_db["get_teacher_by_user_id"].return_value = None
    ...
    async def test_public_with_filters(self, client, override_auth, mock_exams_db):
        ...
        response = await client.get(
            "/api/v1/exams/public?name=maths&type=MIDTERM&subject=MATHS"
        )
        assert response.status_code == 200
        mock_exams_db["get_published_exams"].assert_called_once_with(
            name="maths",
    ...
    async def test_student_exams_not_student(
        self, client, override_auth, mock_exams_db
    ):
        override_auth(role="TEACHER")
    ...
    async def test_get_exam_student_strips_questions(
        self, client, override_auth, mock_exams_db
    ):
        override_auth(role="STUDENT")
    ...
    async def test_patch_not_found(self, client, override_auth, mock_exams_db):
        ...
        response = await client.patch(
            "/api/v1/exams/clxfake_exam_999", json={"name": "Updated Name"}
        )
        assert response.status_code == 404
