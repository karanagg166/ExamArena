...
class TestSchoolsApi:
    ...
    async def test_create_school_success(self, client, override_auth, mock_school_db):
        ...
        # Mock teacher update via join_school
        mock_school_db["join_school"].return_value = fake_teacher
    ...
    async def test_create_school_already_manages(
    ...
    async def test_create_school_missing_fields(
        ...
        payload = {"name": "Test School", "address": "123 Test St", "city": "Mumbai"}
        response = await client.post("/api/v1/schools", json=payload)
        assert response.status_code == 422

    async def test_get_profile_delegates_to_me(
        self, client, override_auth, mock_school_db
    ):
        override_auth(role="PRINCIPAL")
        fake_school = make_fake_school()
    ...
    async def test_update_profile_success(self, client, override_auth, mock_school_db):
        ...
        response = await client.patch(
            "/api/v1/schools/profile", json={"name": "Updated School"}
        )
        assert response.status_code == 200
        assert response.json()["name"] == "Updated School"

    async def test_update_profile_crud_returns_none(
        self, client, override_auth, mock_school_db
    ):
        override_auth(role="PRINCIPAL")
        fake_school = make_fake_school()
    ...
    async def test_classes_for_school_success(
        self, client, override_auth, mock_school_db
    ):
        override_auth(role="STUDENT")
        fake_school = make_fake_school()
        mock_school_db["get_school_by_id"].return_value = fake_school
        mock_school_db["get_school_classes_by_school_id"].return_value = [
            {"id": "class_1", "name": "12th-A"}
        ]
    ...
    async def test_classes_for_school_not_found(
        self, client, override_auth, mock_school_db
    ):
        override_auth(role="STUDENT")
        mock_school_db["get_school_by_id"].return_value = None
