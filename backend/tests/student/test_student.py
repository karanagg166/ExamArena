...
from tests.dummy_data.teachers import make_fake_principal, make_fake_teacher


@pytest.mark.asyncio
class TestStudentsApi:
    # ── Fetch Students ─────────────────────────────────────────
    async def test_fetch_as_student_scoped_to_class(
    ...
    async def test_fetch_as_student_no_profile(
        self, client, override_auth, mock_student_db
    ):
    ...
    async def test_fetch_as_teacher_scoped_to_school(
        self, client, override_auth, mock_student_db
    ):
    ...
    async def test_fetch_as_teacher_no_school(
        self, client, override_auth, mock_student_db
    ):
        override_auth(role="TEACHER")
    ...
    async def test_fetch_as_admin_unscoped(
        self, client, override_auth, mock_student_db
    ):
    ...
    async def test_update_me_success(self, client, override_auth, mock_student_db):
        ...
        updated_student = make_fake_student(
            {"userId": user.id, "parentName": "Updated Parent"}
        )
        mock_student_db["update_student"].return_value = updated_student

        response = await client.put(
            "/api/v1/students/me", json={"parentName": "Updated Parent"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_student_views_own_profile(
        self, client, override_auth, mock_student_db
    ):
        user = override_auth(role="STUDENT")
        fake_student = make_fake_student({"userId": user.id})
    ...
    async def test_student_views_same_class_peer(
        self, client, override_auth, mock_student_db
    ):
        user = override_auth(role="STUDENT")
        me = make_fake_student({"userId": user.id, "classId": "class_123"})
        peer = make_fake_student(
            {"id": "student_peer", "userId": "user_peer", "classId": "class_123"}
        )
    ...
    async def test_student_views_different_class(
        self, client, override_auth, mock_student_db
    ):
        user = override_auth(role="STUDENT")
        me = make_fake_student({"userId": user.id, "classId": "class_123"})
        other = make_fake_student(
            {"id": "student_other", "userId": "user_other", "classId": "class_456"}
        )
        ...
        assert response.status_code == 403
        assert (
            "You can only view profiles of students in your class"
    ...
    async def test_teacher_different_school(
        self, client, override_auth, mock_student_db
    ):
        override_auth(role="TEACHER")
        teacher = make_fake_teacher({"schoolId": "school_123"})
        ...
        assert response.status_code == 403
        assert (
            "You must be a teacher at this student's school"
            in response.json()["detail"]
        )
    ...
    async def test_principal_same_school(self, client, override_auth, mock_student_db):
        ...
        teacher = make_fake_teacher({"schoolId": "school_123"})
        principal = make_fake_principal(
            {"schoolId": "school_123", "teacherId": teacher.id}
        )
        student = make_fake_student({"schoolId": "school_123"})
