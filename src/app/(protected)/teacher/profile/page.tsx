...
export default function TeacherDashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setFormData({
                    qualifications: toStringArray(data.qualifications ?? data.qualification),
    ...
    useEffect(() => {
        api.get("/api/v1/teachers/qualifications")
            .then((r) => setAvailableQualifications(r.data || []))
            .catch(console.error);

        api.get("/api/v1/teachers/subjects")
    ...
    const toggleSelection = (field: "qualifications" | "subjects", value: string) => {
        setFormData((prev) => {
            ...
            return hasValue(selected, value)
                ? { ...prev, [field]: selected.filter((item) => item.toLowerCase() !== value.toLowerCase()) }
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-zinc-400 mt-1">Manage qualifications, subjects, and department.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                    <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-6 max-w-3xl">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-sm text-zinc-400 block mb-2">Qualifications</label>
                                <div className="flex flex-wrap gap-2">
                                    {availableQualifications.length > 0 ? availableQualifications.map((qual) => (
                                        <button
                                            ...
                                            className={`px-3 py-2 rounded-lg text-sm border transition-colors ${hasValue(formData.qualifications, qual)
                                        ...
                                        <p className="text-sm text-zinc-500">No qualifications available.</p>
                                    )}
                                </div>
                            </div>
                            <div>
                                <label className="text-sm text-zinc-400">Experience (years)</label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label className="text-sm text-zinc-400 block mb-2">Subjects</label>
                                <div className="flex flex-wrap gap-2">
                                    {availableSubjects.length > 0 ? availableSubjects.map((subject) => (
                                        <button
                                            ...
                                            className={`px-3 py-2 rounded-lg text-sm border transition-colors ${hasValue(formData.subjects, subject)
                                                ? "bg-indigo-600/20 text-indigo-300 border-indigo-500/50"
                                                : "bg-zinc-900 text-zinc-400 border-zinc-800 hover:bg-zinc-800"
                                                } disabled:opacity-50 disabled:cursor-not-allowed`}
                                        >
                                        ...
                                        <p className="text-sm text-zinc-500">No subjects available.</p>
                                    )}
                                </div>
                            </div>
                        </div>
