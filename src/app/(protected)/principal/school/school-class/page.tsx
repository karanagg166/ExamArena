...
export default function PrincipalSchoolClassPage() {
    ...
    const { classes, loading: classesLoading, fetchClasses } = useSchoolClassStore();
    ...
    useEffect(() => {
        if (school?.id) fetchClasses(school.id);
    ...
    if (!school) {
        return (
            <div className="flex min-h-[80vh] items-center justify-center">
                <p className="text-zinc-500">No school found. Please set up your school first.</p>
            </div>
        );
    }
    ...
    return (
        <div className="page-shell text-white p-6 max-w-6xl mx-auto space-y-8">
            ...
            <Card className="border-zinc-800 bg-zinc-950/50">
                <CardHeader>
                    <CardTitle className="text-3xl font-bold tracking-tight">Classes Directory</CardTitle>
                    <CardDescription className="text-zinc-400">
                        Overview of all classes in {school.name}. Click into a class to view its specific details and student roster.
                    </CardDescription>
                </CardHeader>
            </Card>
