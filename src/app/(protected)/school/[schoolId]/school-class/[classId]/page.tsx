...
export default function ClassPage() {
    ...
    useEffect(() => {
        const fetchClass = async () => {
            ...
            setError(null);
            try {
                const { data } = await api.get<SchoolClass>(`/api/v1/classes/${classId}`);
                setSchoolClass(data);
            } catch (err: unknown) {
            ...
            <Card className="max-w-3xl space-y-6">
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid gap-2 text-sm text-zinc-300 sm:grid-cols-2">
                        <p><span className="text-zinc-500">Class ID:</span> {schoolClass.id}</p>
                        <p><span className="text-zinc-500">School ID:</span> {schoolClass.schoolId}</p>
                        <p><span className="text-zinc-500">Created:</span> {new Date(schoolClass.createdAt).toLocaleDateString()}</p>
                        <p><span className="text-zinc-500">Updated:</span> {new Date(schoolClass.updatedAt).toLocaleDateString()}</p>
                    </div>
